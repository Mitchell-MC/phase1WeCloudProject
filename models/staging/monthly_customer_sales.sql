{{
    config(
        ...
        unique_key=['customer_sk', 'yr_mnth_num', 'sales_category'],
...
from all_sales
left join {{source('airbyte_raw','date_dim')}} d on sold_date_sk = d.D_DATE_SK
{% if is_incremental() %}
where d.yr_mnth_num > '{{MAX_SOLD_YR_MNTH}}'
{% endif %}
group by 1, 2, 3, 4, 5